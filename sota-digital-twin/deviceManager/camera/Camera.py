...
class Camera():
    ...
    def __init__(self, ip, port, cameraid=None, token=None):
        ...
        self.token = self.setSessionId(value=token)
    ...
    def processRequest(self,handler):
        try:
            ...
            op.printLog(logType="EXCEPTION", e=e, messageStr="\n BaseCamera.loadRequest(request)")
            traceback.print_exc()
    ...
    def addInputBuffer(self,value):
        ...
        if (not res):
            op.printLog(logType="WARNING",messageStr="Something went wrong! BaseCamera.addInputBuffer()")            
    ...
    def addInputMessage(self,value):
        ...
        if (not res):
            op.printLog(logType="WARNING",messageStr="Something went wrong! BaseCamera.addInputMessage()")            
    ...
    def addOutputMessage(self,value):
        ...
        if (not res):
            op.printLog(logType="WARNING",messageStr="Something went wrong! BaseCamera.addOutputMessage()")             
    ...
    def addOutputBuffer(self,value):
        ...
        if (not res):
            op.printLog(logType="WARNING",messageStr="Something went wrong! BaseCamera.addOutputBuffer()")
            return res
        self.messagesSent+=1 
    ...
    def toString(self):
        ...
        if(self.token != None):
            tmpString += "logged in with token = ["+self.token+"] "
