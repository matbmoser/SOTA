class RTJPClient{
    constructor(clientid) {
        ...
        this.token = null;
        this.RTJPHandler = new RTJPHandler(this)
    }
    async start(ip, port){
        setConnecting()
        this.serverip = ip;
        ...
        let protocolPayload = ["RTJP",encodeURIComponent(this.RTJPHandler.getSYN())];
    ...
    async sendMessageToClient(clientid, message){
        this.sendMessageToServer(this.RTJPHandler.getMSG(clientid, message))
    }
    ...
    async parseMessage(message){
        try{
            ...
            if("clientid" in this.parsedMessage){
                writeToScreenSpan("<span style='color:"+color+"'><strong>["+this.parsedMessage["clientid"]+"] says -> </strong>"+ this.parsedMessage["message"]+"</span>");
